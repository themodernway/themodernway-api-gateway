...
@RequestBinding('/hazelcast/hzmap')
class HazelcasMaptService extends RESTServiceSupport implements HazelcastTrait
{
    @Override
    def call(final IRESTRequestContext context, final JSONObject object) throws Exception
    {
        def imap = getIMap('imap')

        imap.set(uuid(), new Date().toString())
